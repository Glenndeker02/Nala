...
export default function CampaignDetailsPage() {
    ...
    const fetchCampaignDetails = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Calculate stats
                const videosCount = data.campaign?._count?.videos || 0;
                const applicationsCount = data.campaign?._count?.applications || 0;
                setStats({
                    totalApplications: applicationsCount,
                    pendingReviews: 0, // Would come from API
                    approvedVideos: data.campaign?.videosCompleted || 0,
                    totalViews: 0, // Would come from API
                    budgetSpent: (data.campaign?.videosCompleted || 0) * (data.campaign?.baseFeePerVideo || 0)
                });
            }
        } catch (error) {
    ...
    const baseFeeTotal = campaign.videosRequested * campaign.baseFeePerVideo;
    const performanceBudget = campaign.totalBudget - baseFeeTotal;
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="space-y-4">
                                            ...
                                            {campaign.briefData.talkingPoints && campaign.briefData.talkingPoints.length > 0 && (
                                                <div>
                                                    <p className="text-sm font-medium text-gray-900 mb-2">Key Talking Points</p>
                                                    <ul className="list-disc list-inside space-y-1">
                                                        {campaign.briefData.talkingPoints.map((point: string, index: number) => (
                                                            <li key={index} className="text-gray-600">{point}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                            )}
                                            {campaign.briefData.hashtags && (
                                                <div>
                                                    <p className="text-sm font-medium text-gray-900 mb-1">Hashtags</p>
                                                    <p className="text-gray-600">{campaign.briefData.hashtags}</p>
                                                </div>
                                            )}
                                        </div>
                                    </CardContent>
